...
**Two different failure postures, one per layer.** Within a single successfully-parsed verdict, anything not literally `OK` (`_LLM_OK_VERDICTS = frozenset({"OK"})`, matched case-insensitively) is treated as a violation for that ADR — a stray "MAYBE" or "UNSURE" fails closed. Above that, a call that cannot be parsed at all, times out, or the tool cannot even reach — never contributes a partial verdict: `run_llm_batch` returns `None` for the *whole* pass on any single failure, falling back to declarative-only, because a partially-evaluated pass reported as complete is the exact failure mode TASK-63 exists to remove.
