...
- **Context lookup (`/adr-kit:context`, `bin/adr-context`)**: give it a topic ("mqtt discovery", "caching") and it returns the 3 to 5 most relevant ADRs with a one-line decision summary, ranked by five weighted signals. Accepted ADRs receive a ranking boost, but v0.33.0 can also return Proposed ADRs and does not include status in its result. Treat the result as a shortlist to inspect, not an approval signal.
