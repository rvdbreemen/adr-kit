...
| `/adr-kit:related [ADR-NNN]` | deliberate check | yes | Show the dependency graph for one ADR (v0.28.0+): outbound edges (Related Decisions, Supersedes claims, Superseded by / Amended by status refs) and inbound edges (everything referencing it, including plain mentions). Flags dangling references. Read-only; wraps `bin/adr-related`. |
| `/adr-kit:supersede [ADR-NNN]` | guided write | no | Replace a decision without rewriting history (v0.28.0+). Shows the target's graph first, drafts the superseding ADR via `adr-generator` (Proposed), and only after your approval flips the old Status line to `Superseded by ADR-M` and appends status_history on both sides. Verifies with `bin/adr-related` + `bin/adr-lint`. Refuses to overwrite an existing supersession pointer. |
...
**Trend history (v0.29.0+):** every `adr-guardian stamp` appends a sweep entry to an append-only `trend` list in the state file (date, tier, total ADRs, drift violations, retire candidates, suggest hits, audit findings, Enforcement coverage percent), capped at 52 entries. Once two or more sweeps are recorded, the SessionStart nudge adds a one-line delta vs the previous sweep (`trend: drift 2 -> 0, retire 1 -> 2, coverage 40% -> 45%`). The coverage KPI itself comes from `bin/adr-status`, whose summary now reports `coverage_pct` and `llm_judge_pct` in all three output formats.
