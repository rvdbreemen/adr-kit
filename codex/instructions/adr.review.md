...
- **`"llm_judge": true`.** Free-form ADRs whose rules cannot be expressed as regex but are still code-relevant. The pre-commit hook batches all `llm_judge` ADRs into ONE Claude Sonnet call and blocks the commit on `VIOLATION` when the LLM pass is active. As of v0.17.0 the LLM pass is opt-in (off by default); enable via `judge.llm_enabled:true` in `.adr-kit.json`, or `ADR_KIT_LLM=1` for a single commit. `/adr-kit:judge` always runs the LLM pass on demand regardless of that setting. Falls back to declarative-only when the `claude` CLI is missing — never blocks on tooling drift. Cost shape when enabled: ~$0.10–0.30 per commit on Sonnet 4.6 with prompt caching.
