...
def test_support_matrix_never_claims_a_fail_closed_edit_tier():
    """ADR-004 rejected a fail-closed edit gate; the document may not grant one.

    It granted one for several releases. A hardcoded paragraph called the
    pre-edit tier "the *fail-closed* floor of the injection model" and headed a
    section "no fail-closed edit floor on GitHub Copilot CLI" -- describing a
    floor ADR-004 lists under its rejected alternatives, in the document that
    tells users what each client guarantees, generated by the tool whose job is
    catching exactly this (TASK-138).

    The assertions are on the claims rather than on any one paragraph: the
    paragraph is what got rewritten and the claims are what have to stay gone.
    Matching bare words would be simpler and wrong -- the sentence that *denies*
    a fail-closed edit gate necessarily contains the phrase -- so this reads the
    document with its line wrapping collapsed and looks for the assertions.
    """
    text = CERTIFY.support_matrix(passing())
    flat = " ".join(text.split()).lower()
    for claim in (
        "names the pre-edit tier the *fail-closed* floor",
        "fail-closed floor of the injection model",
        "no fail-closed edit floor",
        "fail-closed edit tier",
    ):
        assert claim not in flat, (
            f"the generated support matrix claims {claim!r}. ADR-004 rejected a "
            "fail-closed edit gate by name: injection hooks steer, they do not "
            "block, and the absence of a pre-edit floor is not a degradation."
        )
    assert "it rejected a fail-closed edit gate by name" in flat
    assert (
        "there is no pre-edit floor on any client, and the absence of one is "
        "not a degradation" in flat
    )
    # The two tiers that do block are named, and the second one is qualified per
    # client rather than asserted flatly (ADR-023).
    assert "## Where enforcement is fail-closed" in text
    assert "commit tier" in flat and "pull-request tier" in flat
    assert "advisory only (`codex-pr-guard-advisory-only`)" in text
    assert "no native event: the commit tier is the only floor here" in text


def test_support_matrix_claims_no_degradation_that_is_not_declared():
    """Every degradation in the document comes from the capability registry."""
    text = CERTIFY.support_matrix(passing())
    declared = {
        entry["id"]: entry
        ...
        for entry in client.get("degradations", [])
    }
    assert declared, "capability registry declares no degradations to render"
    for entry in declared.values():
        assert entry["reason"] in text
        assert entry["user_effect"] in text
